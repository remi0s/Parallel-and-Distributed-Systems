...
for N in 1048576 2097152 4194304 8388608 16777216 33554432 ; do \
    for P in 128 ; do \
		...
		for T in 2 4 8 16 32 64 128 256 512 1024 2048; do \
          		  echo sphere N=$N Threads=$T && ./test_octree $N 1 $P 3 $L $T >> $FILE_PREF-sphere.txt ; \
		done ; \
	done ; \
    done ; \
done ;
...
for N in 1048576 2097152 4194304 8388608 16777216 33554432 ; do \
    for P in 128 ; do \
        for L in 18 ; do \
		for T in 2 4 8 16 32 64 128 256 512 1024 2048; do \
	           echo cube N=$N Threads=$T && ./test_octree $N 0 $P 3 $L $T >> $FILE_PREF-cube.txt ; \
		done ; \
        done ; \
    done ; \
done ;
