...
void morton_encoding(unsigned long int *mcodes, unsigned int *codes, int N, int max_level){
  ...
  int size=N/NUM_THREADS; //it will be used so each thread take this size of chunk
  int i=0;
  //                   declare of share values and private  , give the number of threads, dynamic so each thread that ends may take leftofvers to work
  //dynamic in this case wont be used because N is equally splited    and size is the size of N each thread will work on
  #pragma omp parallel for shared(mcodes, codes) private(i) num_threads(NUM_THREADS) schedule(dynamic, size)
  for(i=0; i<N; i++){
